...
function buildPrompt(goalType: string, timeframe: string, gender: string): string {
  const timeframeText =
  ...
  return `Same person after ${timeframeText} of gym training. Body is now ${bodyText}. Keep background and clothing the same. Photorealistic photo.`
}
...
export async function POST(request: NextRequest) {
  ...
  try {
    step1 = await fal.subscribe('fal-ai/flux-pro/kontext', {
      input: {
        ...
        prompt: buildPrompt(goalType, timeframe, gender),
        guidance_scale: 4.5,
        num_images: 1,
        safety_tolerance: '6',
        output_format: 'jpeg',
  ...
  // Upload to Supabase Storage: goal-physique/{user_id}/goal.jpg
  const storagePath = `${user.id}/goal.jpg`
  ...
  await supabase
    .from('profiles')
    .update({
      goal_image_url: signedData.signedUrl,
      ...
      goal_generated_at: new Date().toISOString(),
    })
    .eq('id', user.id)
